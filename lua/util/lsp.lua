...
function M.on_attach(args, bufnr)
  ...
  keymap(
    ...
    { buffer = bufnr, desc = LSP_DESC_PREFFIX .. 'code action' }
  )
  ...
  keymap(normalMode, ']d', function()
    diagnostic_goto(true)
  end, { buffer = bufnr, desc = LSP_DESC_PREFFIX .. 'Next Diagnostic' })
  keymap(normalMode, '[d', function()
    diagnostic_goto(false)
  end, { buffer = bufnr, desc = LSP_DESC_PREFFIX .. 'Prev Diagnostic' })
  keymap(normalMode, ']e', function()
    diagnostic_goto(true, 'ERROR')
  end, { buffer = bufnr, desc = LSP_DESC_PREFFIX .. 'Next Error' })
  keymap(normalMode, '[e', function()
    diagnostic_goto(false, 'ERROR')
  end, { buffer = bufnr, desc = LSP_DESC_PREFFIX .. 'Prev Error' })
  keymap(normalMode, ']w', function()
    diagnostic_goto(true, 'WARN')
  end, { buffer = bufnr, desc = LSP_DESC_PREFFIX .. 'Next Warning' })
  keymap(normalMode, '[w', function()
    diagnostic_goto(false, 'WARN')
  end, { buffer = bufnr, desc = LSP_DESC_PREFFIX .. 'Prev Warning' })
  --
  -- See `:help K` for why this keymap
  keymap(normalMode, 'K', function()
    vim.lsp.buf.hover({ border = border })
  end, { buffer = bufnr, desc = LSP_DESC_PREFFIX .. 'Hover Documentation' })
  keymap(
    normalMode,
    '<C-k>',
